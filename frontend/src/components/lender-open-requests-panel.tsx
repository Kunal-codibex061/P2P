...
export function LenderOpenRequestsPanel() {
  ...
  const openRequestsQuery = useQuery({
    queryKey: ["lender-open-requests", user?._id],
    queryFn: () => api.get<ItemRequest[]>("/api/lender/open-requests", token),
  ...
  return (
    <section className="space-y-3">
      <div className="flex flex-wrap items-center justify-between gap-2">
        <h2 className="text-lg font-semibold text-slate-900">Nearby Open Requests</h2>
        <Link
          href="/listings/new"
      ...
      {requestOptions.length === 0 ? (
        <div className="rounded-2xl border border-slate-200 bg-white p-4 text-sm text-slate-600">
          No open renter demand nearby right now.
        </div>
      ) : (
        <div className="grid gap-3">
          {requestOptions.slice(0, 6).map((request) => (
